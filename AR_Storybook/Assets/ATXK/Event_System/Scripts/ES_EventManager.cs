namespace ATXK.EventSystem
{
	...
	public class ES_EventManager : SingletonBehaviourless<ES_EventManager>
	{
		...
		public static bool AddEvent(string eventName)
		{
			if(!Instance.events.ContainsKey(eventName))
			{
				...
				DebugLogger.Log<ES_EventManager>("Succesfully registered new ES_GameEvent with name '" + eventName + "'.");
				return true;
			}

			DebugLogger.Log<ES_EventManager>("Failed to register new ES_GameEvent with name '" + eventName + "'. Event already exists within dictionary.");
			return false;
		}
		...
		public static bool RemoveEvent(string eventName)
		{
			if (Instance.events.ContainsKey(eventName))
			{
				...
				DebugLogger.Log<ES_EventManager>("Succesfully unregistered ES_GameEvent with name '" + eventName + "'.");
				return true;
			}

			DebugLogger.Log<ES_EventManager>("Failed to unregister new ES_GameEvent with name '" + eventName + "'. Event does not exist within dictionary.");
			return false;
		}
		...
		public static void RemoveAllEvents()
		{
			...
			foreach (KeyValuePair<string, ES_GameEvent> gameEvent in Instance.events)
			{
				...
				DebugLogger.Log<ES_EventManager>("Removed all Listeners from ES_GameEvent '" + gameEvent + "'.");
			}

			//Clear the dictionary
			Instance.events.Clear();

			DebugLogger.Log<ES_EventManager>("Removed all ES_GameEvent from EventManager registry.");
		}
		...
		public static void StartListening(string eventName, ES_GameEventListener listener)
		{
			...
			if (Instance.events.TryGetValue(eventName, out gameEvent))
			{
				...
				DebugLogger.Log<ES_EventManager>("Succesfully registered new listener to ES_GameEvent with name '" + eventName + "'.");
				return;
			}
		...
		public static void StopListening(string eventName, ES_GameEventListener listener)
		{
			...
			if (Instance.events.TryGetValue(eventName, out gameEvent))
			{
				...
				DebugLogger.Log<ES_EventManager>("Succesfully unregistered listener to ES_GameEvent with name '" + eventName + "'.");
				return;
			}
		}
		...
		public static void CallEvent(string eventName)
		{
			...
			if (Instance.events.TryGetValue(eventName, out gameEvent))
			{
				...
				DebugLogger.Log<ES_EventManager>("Invoked the ES_GameEvent with name '" + eventName + "'.");
				return;
			}
			DebugLogger.Log<ES_EventManager>("ES_GameEvent with name '" + eventName + "' does not exist.");
		}
	}
}